import { ShaderCache } from "./Renderer/shader_cache";
...
class iblSampler {
  ...
  private ggxSampleCount: number;
  private lambertianSampleCount: number;
  private sheenSamplCount: number;
  private lodBias: number;
  private lowestMipLevel: number;
  private lutResolution: number;
  ...
  private cubemapTextureID;
  private framebuffer;

  private shaderCache: ShaderCache;
  private use8bit: boolean = false;
  ...
  loadTextureHDR(image) {
    ...
    if (
      ...
      typeof this.gl.RGB32F !== "undefined"
    ) {
