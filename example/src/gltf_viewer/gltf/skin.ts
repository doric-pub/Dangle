...
class gltfSkin extends GltfObject {
  ...
  private jointTextureInfo;
  private jointWebGlTexture;
  private jointMatrices?: any[];
  private jointNormalMatrices?: any[];
  ...
  computeJoints(gltf, parentNode, webGlContext) {
    const ibmAccessor =
      gltf.accessors[this.inverseBindMatrices].getDeinterlacedView(gltf);
    this.jointMatrices = [];
    ...
    for (const joint of this.joints) {
      ...
      let ibm = jsToGlSlice(ibmAccessor, i * 16, 16) as mat4;
