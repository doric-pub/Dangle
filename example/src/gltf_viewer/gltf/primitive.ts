...
import { gltfTexture, gltfTextureInfo } from "./texture";
...
class gltfPrimitive extends GltfObject {
  ...
  private glAttributes: any[];
  private morphTargetTextureInfo;
  private defines: any[];
  private skip: boolean;
  private hasWeights: boolean;
  private hasJoints: boolean;
  private hasNormals: boolean;
  private hasTangents: boolean;
  private hasTexcoord: boolean;
        ...
        for (let i = 0; i < targetCount; ++i) {
          ...
          for (let [attributeName, offsetRef] of Object.entries(
            ...
            if (target[attributeName] != undefined) {
              ...
              const offset = (offsetRef as number) * singleTextureSize;
              if (
                accessor.componentType != GL.FLOAT &&
                accessor.normalized == false
              ) {
                console.warn("Unsupported component type for morph targets");
                attributeOffsets[attributeName] = (offsetRef as number) + 1;
                continue;
              }
  ...
  decodeGeometry(draco, decoder, decoderBuffer, gltfDracoAttributes, gltf) {
    ...
    for (let dracoAttr in gltfDracoAttributes) {
      ...
      for (const [key, value] of Object.entries(this.attributes)) {
        if (key === dracoAttr) {
          componentType = gltf.accessors[value as any].componentType;
          accessotVertexCount = gltf.accessors[value as any].count;
          break;
        }
      }
      ...
      componentType = this.getDracoArrayTypeFromComponentType(
        componentType
      ) as any;
    ...
    if (geometryType === draco.TRIANGULAR_MESH) {
      ...
      geometry.index = { array: index, itemSize: 1 } as any;
    }
