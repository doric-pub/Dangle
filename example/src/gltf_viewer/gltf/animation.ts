...
import { gltfAnimationChannel, InterpolationPath } from "./channel";
import { gltfAnimationSampler } from "./animation_sampler";
import { gltfInterpolator } from "./interpolator";
...
class gltfAnimation extends GltfObject {
  ...
  private disjointAnimations: any[];
    ...
    for (let i = 0; i < this.interpolators.length; ++i) {
      ...
      switch (channel.target.path) {
        case InterpolationPath.TRANSLATION:
          node.applyTranslationAnimation(
        ...
        case InterpolationPath.WEIGHTS: {
          ...
          mesh.weightsAnimated = interpolator.interpolate(
            gltf,
            channel,
            sampler,
            totalTime,
