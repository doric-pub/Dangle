...
class gltfAccessor extends GltfObject {
  ...
  private glBuffer;
  private typedView;
  private filteredView;
  private normalizedFilteredView;
  private normalizedTypedView;
  ...
  getTypedView(gltf) {
    ...
    if (this.bufferView !== undefined) {
      ...
      let componentCount = this.getComponentCount(this.type) as number;
  ...
  getDeinterlacedView(gltf) {
    ...
    if (this.bufferView !== undefined) {
      ...
      const componentSize = this.getComponentSize(this.componentType);
      const componentCount = this.getComponentCount(this.type) as number;
      ...
      for (let i = 0; i < arrayLength; ++i) {
        let offset =
          Math.floor(i / componentCount) * stride +
          (i % componentCount) * componentSize;
    ...
    if (this.filteredView === undefined) {
      console.warn(
        "Failed to convert buffer view to filtered view!: " + this.bufferView
      );
    } else if (this.sparse !== undefined) {
  ...
  applySparse(gltf, view) {
    ...
    const indicesByteOffset =
      this.sparse.indices.byteOffset + indicesBufferView.byteOffset;
    ...
    const valuesByteOffset =
      this.sparse.values.byteOffset + valuesBufferView.byteOffset;

    const valuesComponentSize = this.getComponentSize(this.componentType);
    let valuesComponentCount = this.getComponentCount(this.type) as number;
