...
import { getIsGlb, getContainingFolder } from "../gltf/utils";
import { GlbParser } from "./glb_parser";
import { gltfLoader } from "./loader";
import { gltfImage, ImageMimeType } from "../gltf/image";
import { gltfTexture, gltfTextureInfo } from "../gltf/texture";
import { gltfSampler } from "../gltf/sampler";
...
class ResourceLoader {
  ...
  async loadGltf(gltfFile, externalFiles) {
    ...
    if (typeof gltfFile === "string") {
      ...
      const resource = new AssetsResource(gltfFile);
      const arrayBuffer = await resourceLoader(context).load(resource);
      let data;
      if (isGlb) {
        data = arrayBuffer;
      } else {
    ...
    if (isGlb) {
      ...
      const glb = glbParser.extractGlbData() as any;
      json = glb.json;
      buffers = glb.buffers;
    }
  ...
  async loadEnvironment(environmentFile, lutFiles) {
    ...
    if (typeof environmentFile === "string") {
      const resource = new AssetsResource(environmentFile);
      const arrayBuffer = await resourceLoader(context).load(resource);
      image = await loadHDR(new Uint8Array(arrayBuffer));
      ...
      image = await loadHDR(new Uint8Array(imageData as ArrayBuffer));
    } else {
      // TODO
      console.error(
        //@ts-ignore
...
async function _loadEnvironmentFromPanorama(imageHDR, view, luts) {
  ...
  const environment = new glTF(undefined);
  ...
  await gltfLoader.loadImages(environment, undefined);
