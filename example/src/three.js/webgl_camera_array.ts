...
@Entry
class webgl_camera_array extends Panel {
  onShow() {
    navbar(context).setTitle("webgl_camera_array");
  }
          ...
          dangleView({
            onReady: (gl: DangleWebGLRenderingContext) => {
              ...
              function init() {
                ...
                const WIDTH =
                  (window.innerWidth / AMOUNT) * window.devicePixelRatio;
                const HEIGHT =
                  (window.innerHeight / AMOUNT) * window.devicePixelRatio;
                ...
                for (let y = 0; y < AMOUNT; y++) {
                  for (let x = 0; x < AMOUNT; x++) {
                    ...
                    (<any>subcamera).viewport = new THREE.Vector4(
                      Math.floor(x * WIDTH),
                      Math.floor(y * HEIGHT),
                      Math.ceil(WIDTH),
                      Math.ceil(HEIGHT)
