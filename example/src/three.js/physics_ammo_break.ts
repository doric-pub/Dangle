...
@Entry
class physics_ammo_break extends Panel {

  private gestureView?: GestureContainer

  onShow() {
    navbar(context).setTitle("physics_ammo_break");
  }
    ...
    this.gestureView.addChild(
      dangleView({
        onReady: (gl: DangleWebGLRenderingContext) => {
          ...
          // } );
          Promise.resolve(Ammo).then(function ( api ) {
            init();
            animate();
          } ).catch((error)=> {
            loge(error)
          ...
          function createObjects() {
            ...
            textureLoader.load( "https://raw.githubusercontent.com/mrdoob/three.js/master/examples/textures/grid.png", function ( texture ) {
          ...
          function initInput() {
            ...
            self.gestureView!!.onTouchDown = (event) => {
              mouseCoords.set(
                ( event.x * Environment.screenScale / window.innerWidth ) * 2 - 1,
                - ( event.y * Environment.screenScale / window.innerHeight ) * 2 + 1
              );
          ...
          function updatePhysics( deltaTime ) {
            ...
            for ( let i = 0, il = rigidBodies.length; i < il; i ++ ) {
              ...
              const objThree = rigidBodies[ i ] as any;
            ...
            for ( let i = 0, il = dispatcher.getNumManifolds(); i < il; i ++ ) {
              ...
              if ( breakable0 && ! collided0 && maxImpulse > fractureImpulse ) {
                ...
                for ( let j = 0; j < numObjects; j ++ ) {
                  ...
                  const fragment = debris[ j ] as any;
